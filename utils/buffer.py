...
class ReplayBuffer:
    def __init__(
        ...
        self.observation_variances = np.zeros((self._max_size,) + self.obs_shape, dtype=np.float32)
        self.next_observation_variances = np.zeros((self._max_size,) + self.obs_shape, dtype=np.float32)
    ...
    def add_batch(
        self,
        obss: np.ndarray,
        obs_variances: np.ndarray,
        next_obss: np.ndarray,
        next_obs_variances: np.ndarray,
        actions: np.ndarray,
        rewards: np.ndarray,
        ...
        self.observation_variances[indexes] = np.array(obs_variances).copy()
        self.next_observations[indexes] = np.array(next_obss).copy()
        self.next_observation_variances[indexes] = np.array(next_obs_variances).copy()
    ...
    def sample(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "observation_variances": torch.tensor(self.observation_variances[batch_indexes]).to(self.device),
            ...
            "next_observation_variances": torch.tensor(self.next_observation_variances[batch_indexes]).to(self.device),
    ...
    def sample_all(self) -> Dict[str, np.ndarray]:
        return {
            ...
            "observation_variances": self.observation_variances[:self._size].copy(),
            "actions": self.actions[:self._size].copy(),
            "next_observations": self.next_observations[:self._size].copy(),
            "next_observation_variances": self.next_observation_variances[:self._size].copy(),
